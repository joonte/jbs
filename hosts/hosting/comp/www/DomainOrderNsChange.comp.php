...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
// возможно слишком быстро меняют, после регистрации или предыдущей смены.
if(!$__USER['IsAdmin'] && Time() - $DomainOrder['StatusDate'] < 600)
	return new gException('TOO_FAST_CHANGE_NS','Изменять сервера DNS можно не чаще чем раз в 10 минут');
...
$Comp = Comp_Load('Form/Input',Array('type'=>'button','onclick'=>'DomainOrderNsChange();','value'=>'Изменить'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$Table[] = $Comp;
