...
case 'false':
	...
	if(In_Array($Order['TypeID'],Array('Individual','Juridical')))
		return new gException('USER_CANNOT_REFUND_JURIDICAL','Услуга находится на договоре юрлица, для возврата обратитесь в систему поддержки');
	#-------------------------------------------------------------------------------
	// если это реферал - запрещаем возрат, лучше руками админу и убедится что поратит а не на реквизиты выедет
	// иначе - убыток будет =)
	if(IntVal($GLOBALS['__USER']['OwnerID']) > 100)
		return new gException('REFERAL_CANNOT_REFUND','Операция невозможна, для возврата обратитесь в систему поддержки');
	#-------------------------------------------------------------------------------
	break;
	#-------------------------------------------------------------------------------
case 'true':
