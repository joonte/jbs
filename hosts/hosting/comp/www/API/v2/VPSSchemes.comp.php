...
foreach($VPSSchemes as $VPSScheme){
	...
	$OS = $OSs = $Lines = Array();
	#-------------------------------------------------------------------------------
	// массив операциок с размерами
	$Lines = Explode("\n",$VPSScheme['Params']['DiskTemplate']);
	#-------------------------------------------------------------------------------
	// сортируем, они там вразнобой
	Sort($Lines);
	#-------------------------------------------------------------------------------
	...
	foreach($Lines as $Line){
		...
		if(IsSet($Template[1])){
			...
			if($Template[1] < $HDD){
				...
				$OS[] = $Template[0];
				#-------------------------------------------------------------------------------
				$OSs[] = Array('Name'=>$Template[0],'Size'=>$Template[1]);
				#-------------------------------------------------------------------------------
			}
			#-------------------------------------------------------------------------------
		}else{
			...
			$OS[] = $Template[0];
			$OSs[] = Array('Name'=>$Template[0],'Size'=>-1);
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
		// перебираем массив $OSs строим пути к картинкам
		foreach(Array_Keys($OSs) as $OSsKey){
			#-------------------------------------------------------------------------------
			$OsLine = Explode(' ',$OSs[$OSsKey]['Name']);
			#-------------------------------------------------------------------------------
			...
			#-------------------------------------------------------------------------------
			$OSs[$OSsKey]['Image'] = Styles_Url($SRC);
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
	}
	...
	$VPSScheme['OSs'] = $OSs;
	#-------------------------------------------------------------------------------
	$VPSScheme['MessageUp'] = $VPSScheme['Params']['MessageUp'];
	$VPSScheme['MessageDown'] = $VPSScheme['Params']['MessageDown'];
	#-------------------------------------------------------------------------------
