...
foreach($Accounts as $Account){
	...
	if(IsSet($Usage[$Account['Login']]['Last'])){
		#-------------------------------------------------------------------------------
		Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: Last'));
		...
		foreach(Array_Keys($Usage[$Account['Login']]['Last']) as $Key){
			...
			#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: Graphs 1'));
			// если такого графика в списке нет - пропускаем
			if(!In_Array($Key,Explode(',',$Settings['Graphs'])))
				continue;
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: Graphs 2'));
			if(!IsSet($iParams['Graphs']))
				$iParams['Graphs'] = Array();
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: Graphs 3'));
			if(!IsSet($iParams['Graphs'][$Key]))
				$iParams['Graphs'][$Key] = Array();
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: вносим предыдущие значения'));
			// вносим предыдущие значения
			if(IsSet($Account['Params']['Graphs'][$Key]['Data']))
				$iParams['Graphs'][$Key]['Data'] = $Account['Params']['Graphs'][$Key]['Data'];
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: добавляем текущие значения, время округляем до минуты'));
			...
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: аттрибуты графиков'));
		...
		foreach(Array_Keys($Usage[$Account['Login']]['Graphs']) as $Key){
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: Graphs as %s',$Key));
			...
			if(In_Array($Key,Explode(',',$Settings['Graphs']))){
				...
				if(IsSet($Account['Params']['Graphs'][$Key]['Data']) && SizeOf($Account['Params']['Graphs'][$Key]['Data']) > 0){
					#-------------------------------------------------------------------------------
					Debug(SPrintF('[comp/www/API/UpdateResourcesUsed]: есть графики для %s',$Key));
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	// перебираем параемтры и лимиты в iParams['Last'], вешаем Status
	if(IsSet($iParams['Last']) && SizeOf($iParams['Last'])> 0){
		#-------------------------------------------------------------------------------
		foreach(Array_Keys($iParams['Last']) as $Key){
			#-------------------------------------------------------------------------------
			$iParams['Last'][$Key]['Status'] = 'Ok';
			#-------------------------------------------------------------------------------
			// проверяем, есть ли в списке того, о чём алертим, этот параметр
			if(!In_Array($Key,Explode(',',$Settings['Status'])))
				continue;
			#-------------------------------------------------------------------------------
			// и что лимит не нулевой
			if(!$iParams['Last'][$Key]['Limit'])
				continue;
			#-------------------------------------------------------------------------------
			// 80%
			if(($iParams['Last'][$Key]['Used']/$iParams['Last'][$Key]['Limit'])*100 > 80)
				$iParams['Last'][$Key]['Status'] = 'Warning';
			#-------------------------------------------------------------------------------
			// 90%
			if(($iParams['Last'][$Key]['Used']/$iParams['Last'][$Key]['Limit'])*100 > 90)
				$iParams['Last'][$Key]['Status'] = 'Critical';
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
	}
