...
foreach($Accounts as $Account){
	...
	if(IsSet($Usage[$Account['Login']]['Last'])){
		...
		$InsertIDs = $InsertKeys = Array();
		#-------------------------------------------------------------------------------
		// перебираем последние данные, вносим их и добавляем к графикам
		foreach(Array_Keys($Usage[$Account['Login']]['Last']) as $Key){
			#-------------------------------------------------------------------------------
			// новое, таблица ResourceCharts. вносим вообще всё, что показывать буду выбирать потом
			$iArray = Array(
					'CreateDate'	=> Round(Time()/60)*60,
					'OrderID'	=> $Account['OrderID'],
					'GraphID'	=> $Key,
					'Data'		=> Is_Null($Usage[$Account['Login']]['Last'][$Key]['Used'])?0:$Usage[$Account['Login']]['Last'][$Key]['Used'],
					'Limit'		=> Is_Null($Usage[$Account['Login']]['Last'][$Key]['Limit'])?0:$Usage[$Account['Login']]['Last'][$Key]['Limit'],
					);
			#-------------------------------------------------------------------------------
			$IsInsert = DB_Insert('ResourceCharts',$iArray);
			...
			#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
			// новое, удаляем элементы для которых графики строим
			Array_Pop($InsertIDs);
			Array_Pop($InsertKeys);
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		// удаляем из БД лишние значения
		if(SizeOf($InsertIDs) > 0 && SizeOf($InsertKeys) > 0){
			#-------------------------------------------------------------------------------
			$Where = Array(
					SPrintF('`ID` NOT IN (%s)',Implode(',',$InsertIDs)),
					SPrintF('`GraphID` IN (%s)',Implode(',',$InsertKeys)),
					SPrintF('`OrderID` = %u',$Account['OrderID']),
					);
			#-------------------------------------------------------------------------------
			$IsDelete = DB_Delete('ResourceCharts',Array('Where'=>$Where));
		...
		foreach(Array_Keys($Usage[$Account['Login']]['Graphs']) as $Key){
			...
			// if(!In_Array($Key,Explode(',',$Settings['Graphs'])) || !IsSet($Account['Params']['Graphs'][$Key]['Data']) || SizeOf($Account['Params']['Graphs'][$Key]['Data']) < 1)
			// continue;
			if(In_Array($Key,Explode(',',$Settings['Graphs']))){
				#-------------------------------------------------------------------------------
				...
				if(IsSet($Account['Params']['Graphs'][$Key]['Data']) && SizeOf($Account['Params']['Graphs'][$Key]['Data']) > 0){
					#-------------------------------------------------------------------------------
					// выбираем 10 последних записей для этого графика из БД
					$Where = Array(SPrintF('`OrderID` = %u',$Account['OrderID']),SPrintF('`GraphID` = "%s"',$Key));
					$ResourceCharts = DB_Select('ResourceCharts',Array('ID','CreateDate','Data'),Array('Where'=>$Where,'SortOn'=>'CreateDate','IsDesc'=>TRUE,'Limits'=>Array(0,10)));
					#-------------------------------------------------------------------------------
					switch(ValueOf($ResourceCharts)){
					case 'error':
						return ERROR | @Trigger_Error(500);
					case 'exception':
						#-------------------------------------------------------------------------------
						// если не найдено ничего - вносим все значения как есть в БД
						foreach(Array_Keys($Usage[$Account['Login']]['Graphs'][$Key]['Data']) as $CreateDate){
							#-------------------------------------------------------------------------------
							$iArray = Array(
									'CreateDate'	=> $CreateDate,
									'OrderID'	=> $Account['OrderID'],
									'GraphID'	=> $Key,
									'Data'		=> $Usage[$Account['Login']]['Graphs'][$Key]['Data'][$CreateDate],
									'Limit'		=> $Usage[$Account['Login']]['Graphs'][$Key]['Limit'],
									);
							#-------------------------------------------------------------------------------
							$IsInsert = DB_Insert('ResourceCharts',$iArray);
							if(Is_Error($IsInsert))
					...
					#-------------------------------------------------------------------------------
					// строим массив с данными в БД
					$DataOld = Array();
					#-------------------------------------------------------------------------------
					// заполняем массив данными БД
					foreach($ResourceCharts as $ResourceChart)
						$DataOld[$ResourceChart['CreateDate']] = Array('ID'=>$ResourceChart['ID'],'Data'=>$ResourceChart['Data']);
					#-------------------------------------------------------------------------------
					#-------------------------------------------------------------------------------
					// разворачиваем массив из БД, там обратный порядок времени
					$DataOld = Array_Reverse($DataOld,TRUE);
					#-------------------------------------------------------------------------------
					// строим массив ключей по массиву из БД - нужен для проверки есть ли значение в БД
					$KeysOld = Array_Keys($DataOld);
					#-------------------------------------------------------------------------------
					#-------------------------------------------------------------------------------
					// флажки что есть ключи в массиве, что надо продолжать
					$IsExists = $IsContinue = FALSE;
					#-------------------------------------------------------------------------------
					// проверяем, что в новых данных есть старые ключи
					foreach(Array_Keys($Usage[$Account['Login']]['Graphs'][$Key]['Data']) as $KeyNew)
						if(In_Array($KeyNew,$KeysOld))
							$IsExists = TRUE;
					#-------------------------------------------------------------------------------
					if($IsExists){
						#-------------------------------------------------------------------------------
						// в новом массиве данных есть старые ключи.
						// перебираем его до первого ключа который есть, далее сравниваем и вставляем/обновляем
						// перебираем новый массив, проверяем наличие графика за такое время в старом
						foreach(Array_Keys($Usage[$Account['Login']]['Graphs'][$Key]['Data']) as $KeyNew){
							#-------------------------------------------------------------------------------
							if(In_Array($KeyNew,$KeysOld)){
								#-------------------------------------------------------------------------------
								// ставим флажок что дальнейшие значения мы обрабатываем
								$IsExists = TRUE;
								#-------------------------------------------------------------------------------
								// сравниваем значения данных, если разные - обновляем
								if($Usage[$Account['Login']]['Graphs'][$Key]['Data'][$KeyNew] == $DataOld[$KeyNew]['Data'])
									continue;
								#-------------------------------------------------------------------------------
								// даные разные. обновляем
								$IsUpdate = DB_Update('ResourceCharts',Array('Data'=>$Usage[$Account['Login']]['Graphs'][$Key]['Data'][$KeyNew]),Array('ID'=>$DataOld[$KeyNew]['ID']));
							...
							#-------------------------------------------------------------------------------
							// пропускаем те значения которые до первого вхождения
							if(!$IsExists)
								continue;
							#-------------------------------------------------------------------------------
							...
							$iArray = Array(
									...
									'OrderID'	=> $Account['OrderID'],
									'GraphID'	=> $Key,
									'Data'		=> $Usage[$Account['Login']]['Graphs'][$Key]['Data'][$KeyNew],
									'Limit'		=> $Usage[$Account['Login']]['Graphs'][$Key]['Limit'],
									);
							#-------------------------------------------------------------------------------
							$IsInsert = DB_Insert('ResourceCharts',$iArray);
						...
						#-------------------------------------------------------------------------------
						// в новом массиве нет старых ключей. перебираем новый и вставляем данные
						foreach(Array_Keys($Usage[$Account['Login']]['Graphs'][$Key]['Data']) as $CreateDate){
							#-------------------------------------------------------------------------------
							$iArray = Array(
									'CreateDate'	=> $CreateDate,
