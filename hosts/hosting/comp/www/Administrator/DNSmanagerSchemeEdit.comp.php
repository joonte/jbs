...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'ViewArea','value'=>$DNSmanagerScheme['ViewArea'],'prompt'=>'Область (view) в которой будут размещаться домены клиента. Обратите внимание, что если поле пустое, или, значение отличается от того которое указано в настройках сервера, реселлер будет с выделенным IP, и, ему будет предложено ввести данное значение самостоятельно.'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
