...
if($DNSmanagerSchemeID){
	...
	$DNSmanagerScheme = Array(
				...
				'ServersGroupID'	=> 1,
				'HardServerID'		=> 0,
				'Comment'		=> 'Тариф для заказа с VPS',
				'IsReselling'		=> FALSE,
				'IsActive'		=> TRUE,
				'IsProlong'		=> TRUE,
...
if(Is_Error($DOM->Load('Window')))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$Title = ($DNSmanagerSchemeID?'Редактирование нового тарифа на DNSmanager':'Добавление нового тарифа на DNSmanager');
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Select',Array('name'=>'HardServerID','style'=>'width: 240px','prompt'=>'Для размещения всех заказов этого тарифа на определённом сервере - выберите его из списка. Обратите внимание, что сервер должен быть из той же группы серверов к которой относится тарифный план.'),$Options,$DNSmanagerScheme['HardServerID']);
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','size'=>5,'name'=>'MaxOrders','value'=>$DNSmanagerScheme['MaxOrders'],'prompt'=> 'Максимально возможное число заказов по данному тарифу, на каждого клиента. Используется для создания "триальных" тарифных планов. Для снятия ограничений, введите ноль.'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'Reseller','value'=>$DNSmanagerScheme['Reseller'],'prompt'=>'Реселлер от имени которого, и в view которого будут создаваться аккаунты'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'ViewArea','value'=>$DNSmanagerScheme['ViewArea'],'prompt'=>'Область (view) в которой будут размещаться домены клиента'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'DomainLimit','value'=>$DNSmanagerScheme['DomainLimit'],'prompt'=>'Максимальное количество доменов, которое клиент сможет разместить на сервере. Очень редко когда клиентам требуется более 300 доменов.'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$Table[] = Array('Число доменов',$Comp);
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
if($DNSmanagerSchemeID){
	#-------------------------------------------------------------------------------
	$Comp = Comp_Load('Form/Input',Array('type'=>'checkbox','onclick'=>'form.DNSmanagerSchemeID.value = (checked?0:value);','value'=>$DNSmanagerSchemeID));
	if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	$Div->AddChild($Comp);
...
$Comp = Comp_Load(
		...
		Array(
			'type'		=> 'button',
			'onclick'	=> SPrintF("FormEdit('/Administrator/API/DNSmanagerSchemeEdit','DNSmanagerSchemeEditForm','%s');",$Title),
			'value'		=> ($DNSmanagerSchemeID?'Сохранить':'Добавить')
			)
		);
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
