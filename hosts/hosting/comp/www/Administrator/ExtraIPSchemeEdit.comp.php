...
$Comp = Comp_Load('Form/Select',Array('name'=>'AddressType'),$Options,$ExtraIPScheme['AddressType']);
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
...
$Table[] = Array('Автоматическое подключение/отключение',$Comp);
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Hosting
$hGroups = DB_Select('HostingServersGroups','*');
switch(ValueOf($hGroups)){
	...
	case 'array':
		...
		$Comp = Comp_Load('Form/Select',Array('name'=>'HostingGroupID'),$Options,$ExtraIPScheme['HostingGroupID']);
		if(Is_Error($Comp))
			return ERROR | @Trigger_Error(500);
...
switch(ValueOf($vGroups)){
	...
	case 'array':
		...
		$Comp = Comp_Load('Form/Select',Array('name'=>'VPSGroupID'),$Options,$ExtraIPScheme['VPSGroupID']);
		if(Is_Error($Comp))
			return ERROR | @Trigger_Error(500);
...
switch(ValueOf($dGroups)){
	...
	case 'array':
		...
		$Comp = Comp_Load('Form/Select',Array('name'=>'DSGroupID'),$Options,$ExtraIPScheme['DSGroupID']);
		if(Is_Error($Comp))
			return ERROR | @Trigger_Error(500);
