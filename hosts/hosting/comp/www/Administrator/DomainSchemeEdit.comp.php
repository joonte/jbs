...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'Name','prompt'=>"Доменная зона в которой будут регистрироваться домены по данному тарифу.\nНапример 'su', 'ru', 'рф'",'value'=>$DomainScheme['Name']));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'checkbox','name'=>'IsTransfer','value'=>'yes','prompt'=>'На данный тариф возможен перенос доменов от других регистраторов'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
if($DomainScheme['IsTransfer'])
	$Comp->AddAttribs(Array('checked'=>'yes'));
#-------------------------------------------------------------------------------
$Table[] = Array(new Tag('SPAN',Array('style'=>'cursor:pointer;','onclick'=>'ChangeCheckBox(\'IsTransfer\'); return false;'),'Возможность переноса'),$Comp);
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'SortID','prompt'=>'В каком порядке тарифы оторажаются. Также, используется для выбора того тарифа на который осуществляется перенос, выбирается первый, согласно этой сортировке','value'=>$DomainScheme['SortID']));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
$Table[] = 'Ограничения';
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'MinOrderYears','value'=>$DomainScheme['MinOrderYears']));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'MaxActionYears','value'=>$DomainScheme['MaxActionYears']));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'DaysToProlong','value'=>$DomainScheme['DaysToProlong'],'prompt'=>'За сколько дней до окончания домена его можно продлевать'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'DaysBeforeTransfer','value' => $DomainScheme['DaysBeforeTransfer'],'prompt'=>'Многие регистраторы, ставят ограничение на возможность переноса домена, если домен скоро оканчивается. Обычно, это 60 дней. Для доменов .рф и .ru - 5 дней'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'DaysAfterTransfer','value'=>$DomainScheme['DaysAfterTransfer'],'prompt'=>'Некоторые регистраторы, ставят ограничение на возможность переноса домена, сразу после его регистрации или продления. По прошествии указанного числа дней, перенос возможен'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('type'=>'text','name'=>'MaxOrders','value'=>$DomainScheme['MaxOrders'],'prompt'=>$Messages['Prompts']['MaxOrders']));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
