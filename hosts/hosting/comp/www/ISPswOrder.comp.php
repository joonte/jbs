...
if(!$StepID){
	...
	case 'array':
		...
		foreach($VPSOrders as $VPSOrder){
			...
			$Options[$VPSOrderID]	= SPrintF('%s / %s',$VPSOrder['IP'],$VPSOrder['Login']);
			#-------------------------------------------------------------------------------
			$OrderCount++;
			#-------------------------------------------------------------------------------
		}
		...
		if(Is_Error($Comp))
			return ERROR | @Trigger_Error(500);
		#-------------------------------------------------------------------------------
		// если лицензии есть НЕ на все ВПС, добавляем поле
                if(SizeOf($Options) > 1)
	...
	case 'array':
		...
		foreach($DSOrders as $DSOrder){
			...
			$Options[$DSOrderID]	= SPrintF('%s / %s',$DSOrder['Name'],$DSOrder['IP']);
			#-------------------------------------------------------------------------------
			$OrderCount++;
			#-------------------------------------------------------------------------------
		}
		...
		if(Is_Error($Comp))
			return ERROR | @Trigger_Error(500);
		#-------------------------------------------------------------------------------
		// если лицензии есть НЕ на все выделенные серера, добавляем поле
                if(SizeOf($Options) > 1)
