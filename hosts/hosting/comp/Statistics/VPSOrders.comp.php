...
#-------------------------------------------------------------------------------
$Where = SPrintF('`VPSOrders`.`OrderID` = `Orders`.`ID` AND `VPSOrders`.`StatusID` = "Active" AND `OrderDate` >= %u AND `OrderDate` <= %u',$StartDate,$FinishDate);
#-------------------------------------------------------------------------------
if(In_Array('ByDays',$Details)){
	#-------------------------------------------------------------------------------
	$VPSOrders = DB_Select(Array('Orders','VPSOrders'),Array('COUNT(*) as `Count`','OrderID','OrderDate',' GET_DAY_FROM_TIMESTAMP(`OrderDate`) as `Day`','DATE_FORMAT(FROM_UNIXTIME(`OrderDate`),GET_FORMAT(DATE,"ISO")) AS `ISO_Date`'),Array('Where'=>$Where,'GroupBy'=>'Day','SortOn'=>'OrderDate'));
	#-------------------------------------------------------------------------------
	...
	case 'array':
		...
		foreach($VPSOrders as $VPSOrder){
			#-------------------------------------------------------------------------------
			// график по заказам
			$Graphs['ByDay']['Data'][] = Array($VPSOrder['ISO_Date'],$VPSOrder['Count']);
			#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
...
if(In_Array('ByMonth',$Details)){
	#-------------------------------------------------------------------------------
	$VPSOrders = DB_Select(Array('Orders','VPSOrders'),Array("DATE_FORMAT(FROM_UNIXTIME(`OrderDate`),'%Y-%m') AS `YearMonth`",'OrderID','COUNT(*) as `Count`','MONTH(FROM_UNIXTIME(`OrderDate`)) as `Month`','OrderDate','YEAR(FROM_UNIXTIME(`OrderDate`)) as Year'),Array('Where'=>$Where,'GroupBy'=>Array('Month','Year'),'SortOn'=>'OrderDate'));
	#-------------------------------------------------------------------------------
	switch(ValueOf($VPSOrders)){
	case 'error':
		return ERROR | @Trigger_Error(500);
