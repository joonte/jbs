...
#-------------------------------------------------------------------------------
$Where = SPrintF('`HostingOrders`.`OrderID` = `Orders`.`ID` AND `HostingOrders`.`StatusID` = "Active" AND `OrderDate` >= %u AND `OrderDate` <= %u',$StartDate,$FinishDate);
#-------------------------------------------------------------------------------
if(In_Array('ByDays',$Details)){
	#-------------------------------------------------------------------------------
	$HostingOrders = DB_Select(Array('Orders','HostingOrders'),Array('COUNT(*) as `Count`','OrderID','OrderDate',' GET_DAY_FROM_TIMESTAMP(`OrderDate`) as `Day`','DATE_FORMAT(FROM_UNIXTIME(`OrderDate`),GET_FORMAT(DATE,"ISO")) AS `ISO_Date`'),Array('Where'=>$Where,'GroupBy'=>'Day','SortOn'=>'OrderDate'));
	#-------------------------------------------------------------------------------
	...
	case 'array':
		...
		foreach($HostingOrders as $HostingOrder){
			#-------------------------------------------------------------------------------
			// график по заказам
			$Graphs['ByDay']['Data'][] = Array($HostingOrder['ISO_Date'],$HostingOrder['Count']);
			#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
...
if(In_Array('ByMonth',$Details)){
	#-------------------------------------------------------------------------------
	$HostingOrders = DB_Select(Array('Orders','HostingOrders'),Array("DATE_FORMAT(FROM_UNIXTIME(`OrderDate`),'%Y-%m') AS `YearMonth`",'OrderID','COUNT(*) as `Count`','MONTH(FROM_UNIXTIME(`OrderDate`)) as `Month`','OrderDate','YEAR(FROM_UNIXTIME(`OrderDate`)) as Year'),Array('Where'=>$Where,'GroupBy'=>Array('Month','Year'),'SortOn'=>'OrderDate'));
	#-------------------------------------------------------------------------------
	switch(ValueOf($HostingOrders)){
	case 'error':
		return ERROR | @Trigger_Error(500);
