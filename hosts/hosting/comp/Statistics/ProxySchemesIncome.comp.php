...
#-------------------------------------------------------------------------------
$NoBody->AddChild(new Tag('P','Данный вид статистики содержит информацию о доходности/нагрузке каждого из имеющихся тарифов прокси-серверов за 1 месяц (30 дней)'));
...
$ProxySchemes = DB_Select('ProxySchemes',Array('ID','Name'),Array('SortOn'=>'SortID'));
...
foreach($ProxySchemes as $ProxyScheme){
	#-------------------------------------------------------------------------------
	Debug(SPrintF('[comp/Statistics/ProxySchemesIncome]: processing scheme "%s"',$ProxyScheme['Name']));
	#-------------------------------------------------------------------------------
	# достаём все активные аккаунты тарифа
	$SchemeAccounts = DB_Select('ProxyOrders',Array('OrderID'),Array('Where'=>SPrintF('`SchemeID` = %u AND `StatusID` = "Active"',$ProxyScheme['ID'])));
	#-------------------------------------------------------------------------------
	...
	case 'exception':
		#-------------------------------------------------------------------------------
		Debug(SPrintF('[comp/Statistics/ProxySchemesIncome]: scheme "%s" not have active accounts',$ProxyScheme['Name']));
		#-------------------------------------------------------------------------------
		$SchemeIncome = Comp_Load('Formats/Currency',0);
		if(Is_Error($SchemeIncome))
			return ERROR | @Trigger_Error(500);
		#-------------------------------------------------------------------------------
		$Table[] = Array($ProxyScheme['Name'],'0 / 0',$SchemeIncome);
		#-------------------------------------------------------------------------------
		break;
		#-------------------------------------------------------------------------------
	case 'array':
		...
		case 'exception':
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/Statistics/ProxySchemesIncome]: no summ for scheme "%s"',$ProxyScheme['Name']));
			#-------------------------------------------------------------------------------
			break;
			#-------------------------------------------------------------------------------
		case 'array':
			...
			if($SchemeIncome > 0){
				#-------------------------------------------------------------------------------
				// прибыль с тарифа
				$bGraphs[] = Array($ProxyScheme['Name'],$SchemeIncome);
				#-------------------------------------------------------------------------------
			}
			#-------------------------------------------------------------------------------
			// общее число активных акааунтов
			$dGraphs[] = Array($ProxyScheme['Name'],SizeOf($Array));
			#-------------------------------------------------------------------------------
			#-------------------------------------------------------------------------------
			...
			if(Is_Error($SchemeIncome))
				return ERROR | @Trigger_Error(500);
			#-------------------------------------------------------------------------------
			$Table[] = Array($ProxyScheme['Name'],SPrintF('%s / %s',SizeOf($Array)/* num accounts */,$PaidAccounts),$SchemeIncome);
			#-------------------------------------------------------------------------------
			break;
			#-------------------------------------------------------------------------------
		default:
...
if(SizeOf($dGraphs) > 1){
	#-------------------------------------------------------------------------------
	$Graphs = Array('Количество заказов на прокси-сервера, по тарифам'=>$dGraphs,'Доход тарифных планов на прокси-сервера'=>$bGraphs);
	#-------------------------------------------------------------------------------
