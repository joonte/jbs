...
foreach($ServersGroups as $ServersGroup){
	...
	$iServers = DB_Select('Servers',Array('*'),Array('Where'=>SPrintF('`ServersGroupID` = %u',$ServersGroup['ID'])));
	#-------------------------------------------------------------------------------
	switch(ValueOf($iServers)){
	case 'error':
		return ERROR | @Trigger_Error(500);
	case 'exception':
		#-------------------------------------------------------------------------
	...
	case 'array':
		...
		$Servers = Array();
		#-------------------------------------------------------------------------
		foreach($iServers as $iServer)
			if($iServer['Params']['SystemID'] != 'NullSystem' && $iServer['Params']['IsAutoBalancing'])
				$Servers[] = $iServer;
		#-------------------------------------------------------------------------
		break;
		#-------------------------------------------------------------------------
		...
		if(SizeOf($LA) > 0){
			...
			foreach($Servers as $iServer)
				if($iServer['ID'] == $ServerID)
					$GLOBALS['TaskReturnInfo'][$ServersGroup['Name']] = Array($iServer['Address']);
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
	}
