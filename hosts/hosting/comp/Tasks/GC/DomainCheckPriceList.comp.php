...
foreach($Servers as $Registrator){
	...
	foreach(Array_Keys($Prices) as $Key){
		...
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		// у reg.ru глюк, .site выдал с нулевой ценой регистрации и продления
		if($Prices[$Key]['new'] < 1 || $Prices[$Key]['renew'] < 1){
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/Tasks/GC/DomainCheckPriceList]: слишком низкая цена: s/%s; цена регистрации = %s; цена продления = %s',$Registrator['Params']['Name'],$Key,$Prices[$Key]['new'],$Prices[$Key]['renew']));
			#-------------------------------------------------------------------------------
			continue;
			#-------------------------------------------------------------------------------
		}
