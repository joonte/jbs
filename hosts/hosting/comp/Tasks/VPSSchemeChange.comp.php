...
$Where = Array('`ModeID` = "VPSOrders"',SPrintF('`RowID` = %u',$VPSOrderID));
#-------------------------------------------------------------------------------
...
case 'array':
	...
	foreach($StatusesHistory as $Status){
		#-------------------------------------------------------------------------------
		Debug(SPrintF('[comp/Tasks/VPSSchemeChange]: StatusID = %s',$Status['StatusID']));
		#-------------------------------------------------------------------------------
...
case 'exception':
	...
	#-------------------------------------------------------------------------------
	// проставляем статус
	$Comp = Comp_Load('www/API/StatusSet',Array('ModeID'=>'VPSOrders','StatusID'=>$StatusID,'RowsIDs'=>$VPSOrderID,'Comment'=>$SchemeChange->String));
	#-------------------------------------------------------------------------------
	switch(ValueOf($Comp)){
	case 'error':
		return ERROR | @Trigger_Error(500);
...
case 'true':
	...
	$Comp = Comp_Load('www/API/StatusSet',Array('ModeID'=>'VPSOrders','StatusID'=>$StatusID,'RowsIDs'=>$VPSOrderID,'Comment'=>IsSet($Message)?$Message:'Тарифный план изменен'));
	#-------------------------------------------------------------------------------
	switch(ValueOf($Comp)){
	case 'error':
		return ERROR | @Trigger_Error(500);
