...
foreach($DSServers as $DSServer){
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	// если сейчас два часа дня, понедельник - выгружаем логи
	if(Date('G') == 14 && Date('N') == 1){
		...
		if(Is_Exception($Result)){
			...
			if($Result['Entries'] > 0){
				...
				if(Is_Exception($Result)){
					#-------------------------------------------------------------------------------
					Debug(SPrintF('[comp/Tasks/DSServersIPMI]: в логе сервера %s найдено %s новых записей, но извлечь их не удлаось',$DSServer['Name'],$Result['Entries']));
					#-------------------------------------------------------------------------------
				}else{
					#-------------------------------------------------------------------------------
					...
					if($Settings['IsUserNotify'] && !Is_Null($DSServer['UserID']))
