...
/*
#-------------------------------------------------------------------------------
$ProxySchemes = DB_Select('ProxySchemes',Array('ID','Name','CostMonth','(SELECT `Name` FROM `ServersGroups` WHERE `ProxySchemes`.`ServersGroupID` = `ServersGroups`.`ID`) as `ServersGroupName`'),Array('SortOn'=>'SortID'));
#-------------------------------------------------------------------------------
...
switch(ValueOf($Servers)){
  ...
  case 'array':
    ...
    #---------------------------------------------------------------------------
    if($ServerID != 'Default')
      $AddingWhere[] = SPrintF('(SELECT `ServerID` FROM `OrdersOwners` WHERE `ProxyOrdersOwners`.`OrderID` = `OrdersOwners`.`ID`) = %u',$ServerID);
    #---------------------------------------------------------------------------
