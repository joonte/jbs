...
case 'Jabber':
	#-------------------------------------------------------------------------------
	if(!$Config['Tasks']['Types']['Jabber']['IsActive'])
		break;
	#-------------------------------------------------------------------------------
	$Count = DB_Count('Servers',Array('Where'=>SPrintF('`TemplateID` = "Jabber" AND `IsActive` = "yes" AND `IsDefault` = "yes" AND `ID` != %u',$Server['ID'])));
	#-------------------------------------------------------------------------------
	if(Is_Error($Count))
		return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	if(!$Count)
		return new gException('DELETE_DENIED',SPrintF('Удаление сервера (%s) не возможно, т.к. это единственный или активный по умолчанию сервер для отправки Jabber сообщений. Для возможности удаления, сделайте активным другой сервер с шаблоном (Jabber) или отключите задание (%s)',$Server['Address'],$Config['Tasks']['Types']['Jabber']['Name']));
