...
#-------------------------------------------------------------------------------
if($Count)
	return new gException('DELETE_DENIED',SPrintF('Удаление сервера (%s) невозможно, %u заказ(ов) связаны с данным сервером',$Server['Address'],$Count));
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
if($Server['TemplateID'] == 'EmailClient'){
	...
	#-------------------------------------------------------------------------------
	if(!$Count)
		return new gException('DELETE_DENIED',SPrintF('Удаление сервера (%s) невозможно, т.к. это единственный или активный по умолчанию сервер для приёма почты. Для возможности удаления, сделайте активным другой сервер с шаблоном (EmailClient) или отключите задание (%s)',$Server['Address'],$Config['Tasks']['Types']['CheckEmail']['Name']));
	#-------------------------------------------------------------------------------
}

#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
if(In_Array($Server['TemplateID'],Array('Telegram','VKontakte','Viber','Jabber','SMS'))){
	#-------------------------------------------------------------------------------
