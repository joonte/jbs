...
function WebNames_Get_List_Domains($Settings){
	...
	if(!$Result || SizeOf($Result) < 2){
		...
		$HTTP = WebNames_Build_HTTP($Settings);
		#-------------------------------------------------------------------------------
		$Query = Array('thisPage'=>'pispAllDomainsInfo','username'=>$Settings['Login'],'password'=> $Settings['Password'],'interface_revision'=>1,'interface_lang'=>'en');
		#-------------------------------------------------------------------------------
		$Result = HTTP_Send('/RegTimeSRS.pl',$HTTP,Array(),$Query);
		if(Is_Error($Result))
	...
	$Domains = Explode("\n", $Result);
	#-------------------------------------------------------------------------------
	Debug(SPrintF('[WebNames_Get_List_Domains]: Domains = %s',print_r($Domains,true)));
	#-------------------------------------------------------------------------------
	...
	foreach($Domains as $Domain){
		...
		if(IsSet($DomainInfo[1]) && StrLen(Trim($DomainInfo[1])) > 3)
