...
for($i = 1; $i <= $ServersGroup['Params']['Count']; $i++){
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	Debug(SPrintF('[comp/Triggers/GLOBAL]: Создание заказа на услугу #%u; сервис = %u; тариф = %u; дополнительные параметры = (%s); не дублировать = %s',$i,$ServiceID,$SchemeID,$AdditionalParams,(($IsNoDuplicate)?'TRUE':'FALSE')));
	#-------------------------------------------------------------------------------
	...
	if($AdditionalParams){
		...
		foreach(Explode(",",$AdditionalParams) as $AdditionalParam){
			...
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/Triggers/GLOBAL]: Additional: %s => %s',$Name,$Value));
			#-------------------------------------------------------------------------------
