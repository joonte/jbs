...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
/* в принципе, при ограниченных комбинациях полей у юзеров, этой проверки не надо. а админ и так всё сломает при желании.
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Columns = Array('UserID',$ColumnID);
#-------------------------------------------------------------------------------
if($TableID == 'Contracts')
	$Columns[] = 'TypeID';
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Selected = DB_Select(SPrintF('%sOwners',$TableID),$Columns,Array('UNIQ','ID'=>$RowID));
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
// если это попытка скрытия договора, не партнёрского
if($TableID == 'Contracts' && $Selected['TypeID'] != 'NaturalPartner' && !$Selected[$ColumnID]){
	#-------------------------------------------------------------------------------
	// для договоров, возвращаем ошибку при попытке скрыть единственный нескрытый, не-партнёрский договор
	$Count = DB_Count('ContractsOwners',Array('Where'=>Array(SPrintF('`UserID` = %u',$GLOBALS['__USER']['ID']),'`IsHidden` = "no"','`TypeID` != "NaturalPartner"')));
	if(Is_Error($Count))
        	return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	if($Count == 1)
		return new gException('CANNOT_HIDE_LAST_CONTRACT','Нельзя скрыть единственный активный/нескрытый догвоор');
	#-------------------------------------------------------------------------------
}
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
