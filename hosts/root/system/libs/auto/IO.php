	...
	if(File_Exists($Path)){
		...
		if(!File_Exists($Folder)){
			...
			if(!@MkDir($Folder,0777,TRUE))
				return ERROR | @Trigger_Error(SPrintF('[IO_Write]: невозможно создать директорию (%s)',$Folder));
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
	}
...
function IO_Scan($Path,$IsHidden = TRUE){
	...
	$Folder = @OpenDir($Path);
	if(!$Folder)
		return ERROR | @Trigger_Error(SPrintF('[IO_Scan]: невозможно открыть директорию (%s)',$Path));
	#-------------------------------------------------------------------------------
	$Ignored = Array('.','..');
	#-------------------------------------------------------------------------------
	if($IsHidden)
