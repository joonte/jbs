...
function HTTP_Send($Target,$Settings,$Get = Array(),$Post = Array(),$Addins = Array()){
	...
	$Default = Array(
			...
			'Host'		=> 'localhost',
			'Basic'		=> '',
			'Charset'	=> 'UTF-8',	// далее она захардкожена. тут менять бесполезно
			'Hidden'	=> '',
			'IsLogging'	=> TRUE,
			);
	#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
	// метод, по дефолту GET, если массив и строки в нём есть - POST, если не массив и данные есть - тоже POST
	$Method = 'GET';
	#-------------------------------------------------------------------------------
	if(Is_Array($Post)){
		#-------------------------------------------------------------------------------
	...
	#-------------------------------------------------------------------------------
	#$Method = ((Is_Array($Post) && Count($Post) > 0)?'POST':'GET');
	#-------------------------------------------------------------------------------
	$Hidden = $Default['Hidden'];
	#-------------------------------------------------------------------------------
	if(Count($Get))
		$Target .= SPrintF('?%s',HTTP_Query($Get,($Charset)?$Charset:'UTF-8',$Hidden,$IsLogging));
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	...
	if($Post){
		...
		if(Is_Array($Post)){
			...
			if(Count($Post) > 0){
				...
				#-------------------------------------------------------------------------------
				$Body = HTTP_Query($Post,($Charset)?$Charset:'UTF-8',$Hidden,$IsLogging);
				#-------------------------------------------------------------------------------
			}
			#-------------------------------------------------------------------------------
		}else{
			#-------------------------------------------------------------------------------
			// если кодировка пустая - не перекодируем, это бинарные данные суются
			$Body = ($Charset)?Mb_Convert_Encoding($Post,$Charset):$Post;
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
	}
	...
	if(Preg_Match('/Content-Type:[\sa-zA-Z0-9\/\-\;]+charset\=([a-zA-Z0-9\-]+)/i',$Receive,$Matches)){
		...
		#-------------------------------------------------------------------------------
		$Receive = ($Charset)?Mb_Convert_Encoding($Receive,'UTF-8',$Charset):$Receive;
		#-------------------------------------------------------------------------------
	}
	#-------------------------------------------------------------------------------
	...
	if($Hidden){
		...
		foreach($Hidden as $Pattern){
			#-------------------------------------------------------------------------------
			$Pattern = UrlEncode(Mb_Convert_Encoding($Pattern,($Charset)?$Charset:'UTF-8'));
			#-------------------------------------------------------------------------------
			$Log = Str_Replace($Pattern,SPrintF('[HIDDEN=(%u)]',StrLen($Pattern)),$Log);
			#-------------------------------------------------------------------------------
		}
