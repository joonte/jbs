...
/******************************************************************************/
/******************************************************************************/
$__args_list = Array('MessageID','CreateDate','UserID','OwnerID','Content','IP','UA','IsVisible','VoteBall');
/******************************************************************************/
Eval(COMP_INIT);
/******************************************************************************/
/******************************************************************************/
...
$Files = GetUploadedFilesInfo('EdesksMessages', $MessageID);
#-------------------------------------------------------------------------------
if(SizeOf($Files)){
	#-------------------------------------------------------------------------------
	// перебираем файлы
	foreach($Files as $File){
		...
		if($Mime[0] != 'image')
			continue;
		#-------------------------------------------------------------------------------
		// если у юзера настройка не отображать картинки - пропускаем
		if($__USER['Params']['Settings']['EdeskImagesPreview'] != "Yes")
			continue;
		#-------------------------------------------------------------------------------
...
if(SizeOf($Files)){
	...
	foreach($Files as $File){
		#-------------------------------------------------------------------------------
		// разрешаем удалять админу или юзеру, если это последнее сообщение, и тикеты разрешено редактировать
		if($__USER['IsAdmin'] || (IsSet($LastMessage) && $LastMessage['ID'] == $MessageID && $LastMessage['CreateDate'] > Time() - 24*3600)){
			#-------------------------------------------------------------------------------
			$Delete = SPrintF('<a href="JavaScript:ShowConfirm(\'Вы подтверждаете удаление файла?\',\'AjaxCall(\\\'/API/FileDelete\\\',{FileID:%u},\\\'Удаление файла\\\',\\\'GetURL(document.location);\\\');\');" onmouseover="PromptShow(event,\'Удалить это вложение\',this);">[удалить]</a>',$File['ID']);
			#-------------------------------------------------------------------------------
		}else{
			#-------------------------------------------------------------------------------
			$Delete = '';
...
if($GLOBALS['__USER']['IsAdmin']){
	#-------------------------------------------------------------------------------
	$UserInfoImage = Comp_Load('Users/GeoIP',$IP,TRUE,$UA);
	if(Is_Error($UserInfoImage))
		return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	$Params['UserInfoImage'] = $UserInfoImage;
	#-------------------------------------------------------------------------------
}
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Div = new Tag('DIV',Array('class'=>'EdeskMessage'));
#-------------------------------------------------------------------------------
$Div->AddHTML(TemplateReplace('Edesks.Message',$Params));
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
return $Div;
#-------------------------------------------------------------------------------
