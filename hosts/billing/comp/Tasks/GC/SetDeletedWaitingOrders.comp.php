...
$Config = Config();
#-------------------------------------------------------------------------------
$Settings = $Config['Tasks']['Types']['GC']['SetDeletedWaitingOrdersSettings'];
#-------------------------------------------------------------------------------
...
for($i=0;$i<Count($Services);$i++){
	...
	#-------------------------------------------------------------------------------
	$Where = SPrintF("`StatusID` = 'Waiting' AND `StatusDate` < UNIX_TIMESTAMP() - %d * 86400", $Settings['DaysBeforeDeleted']);
	#-------------------------------------------------------------------------------
	...
	case 'array':
		...
		foreach($Orders as $Order){
			...
			if(Is_Error(DB_Transaction($TransactionID = UniqID('comp/Tasks/GC/SetDeletedWaitingOrders'))))
				return ERROR | @Trigger_Error(500);
			#-------------------------------------------------------------------------------
			$Comp = Comp_Load('www/API/StatusSet',Array('ModeID'=>$Services[$i]['Table'],'StatusID'=>'Deleted','RowsIDs'=>$Order['ID'],'IsNoTrigger'=>'yes','Comment'=>SPrintF('Автоматическая отмена заказа, неоплачен более %d дней', $Settings['DaysBeforeDeleted'])));
			#-------------------------------------------------------------------------------
			switch(ValueOf($Comp)){
			case 'array':
				#-------------------------------------------------------------------------------
				$Event = Array(
						'UserID'	=> $Order['UserID'],
						'PriorityID'	=> 'Billing',
						'Text'		=> SPrintF('Автоматическая отмена заказа (%s) #%d, неоплачен более %d дней.',$Services[$i]['Code'],$Order['OrderID'],$Settings['DaysBeforeDeleted'])
						);
				#-------------------------------------------------------------------------------
...
case 'array':
	...
	foreach($Orders as $Order){
		...
		if(Is_Error(DB_Transaction($TransactionID = UniqID('comp/Tasks/GC/SetDeletedWaitingOrders'))))
			return ERROR | @Trigger_Error(500);
		#-------------------------------------------------------------------------
		$Comp = Comp_Load('www/API/StatusSet',Array('ModeID'=>'Orders','StatusID'=>'Deleted','RowsIDs'=>$Order['ID'],'IsNoTrigger'=>'yes','Comment'=>SPrintF('Автоматическая отмена заказа, неоплачен более %d дней',$Settings['DaysBeforeDeleted'])));
		#-------------------------------------------------------------------------
		switch(ValueOf($Comp)){
		case 'array':
			$Event = Array(
					'UserID'	=> $Order['UserID'],
					'PriorityID'	=> 'Billing',
					'Text'		=> SPrintF('Автоматическая отмена заказа (%s) #%d, неоплачен более %d дней.',$Order['NameShort'],$Order['ID'],$Settings['DaysBeforeDeleted'])
					);
			#-------------------------------------------------------------------------------
			$Event = Comp_Load('Events/EventInsert',$Event);
			if(!$Event)
