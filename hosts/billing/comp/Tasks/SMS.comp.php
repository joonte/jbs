...
if(IsSet($User['Params']['SMSTime']) && !$IsImmediately){
	...
	#-------------------------------------------------------------------------------
	if(IsSet($SMSTime['SMSBeginTime']) && IsSet($SMSTime['SMSEndTime'])){
		#-------------------------------------------------------------------------------
		# если начало-конец совпадают, шлёи круглосуточно
			...
			if($SMSTime['SMSBeginTime'] < $SMSTime['SMSEndTime']){
				...
				if(Date('G') > $SMSTime['SMSBeginTime'] && Date('G') < $SMSTime['SMSEndTime']){
					...
					if(Date('G') < $SMSTime['SMSBeginTime']){
						...
						return MkTime($SMSTime['SMSBeginTime'],0,0,Date('n'),Date('j'),Date('Y'));
						#-------------------------------------------------------------------------------
					}else{
						#-------------------------------------------------------------------------------
						# завтра пораньше
						Debug(SPrintF('[comp/Tasks/SMS]: Перенос отправки сообщения (%u) на завтра, %s:00', $Mobile,$SMSTime['SMSBeginTime']));
						return MkTime($SMSTime['SMSBeginTime'],0,0,Date('n'),Date('j')+1,Date('Y'));
						#-------------------------------------------------------------------------------
					}
				}
				#-------------------------------------------------------------------------------
