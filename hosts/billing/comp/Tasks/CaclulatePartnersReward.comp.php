	...
	foreach($Owners as $Owner){
		...
		case 'array':
			...
			if($TotalSummToUser > 0){
				...
				$IsSend = Notify_Send('PartnersReneward',(integer)$Owner['DistinctOwnerID'],Array('Theme'=>$Theme,'Message'=>$MessageToUser));
				#---------------------------------------------------------
	...
	case 'array':
		...
		foreach($Employers as $Employer){
			...
			if($Employer['ID'] > 2000 || $Employer['ID'] == 100){
				...
				$IsSend = Notify_Send('PartnersReneward',(integer)$Employer['ID'],Array('Theme'=>$Theme,'Message'=>$MessageToAdmins));
				#---------------------------------------------------------
				switch(ValueOf($IsSend)){
				case 'error':
					return ERROR | @Trigger_Error(500);
