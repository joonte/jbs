...
if(IsSet($Params['ExecutePeriod']) && $Params['ExecutePeriod']){
	...
	if(IsSet($Time[1])){
		#-------------------------------------------------------------------------------
		# если задана периодичность - добавляем период, дней
		$ExecuteDay = Date('j') + IsSet($Params['ExecuteDays'])?$Params['ExecuteDays']:1;
		#-------------------------------------------------------------------------------
		# если задан точный день месяца - устанавливаем его
		$ExecuteDay = IsSet($Params['ExecuteDayOfMonth'])?$Params['ExecuteDayOfMonth']:$ExecuteDay;
		#-------------------------------------------------------------------------------
		if((integer)$Time[0] >= 0 && (integer)$Time[0] <= 23 && (integer)$Time[1] >= 0 && (integer)$Time[1] <= 59){
			#-------------------------------------------------------------------------------
			return MkTime((integer)$Time[0],(integer)$Time[1],0,Date('n'),$ExecuteDay,Date('Y'));
			#-------------------------------------------------------------------------------
		}else{
			#-------------------------------------------------------------------------------
			return $Params['DefaultTime'];
