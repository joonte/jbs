...
if(!$IsCreate)
  return $Result;
#-------------------------------------------------------------------------------
$Contracts = DB_Select('Contracts',Array('SUM(`Balance`) as `Balance`','TypeID','COUNT(*) as `Count`'),Array('Where'=>"EXISTS(SELECT * FROM `Invoices` WHERE `ID` = `Contracts`.`ID` AND `IsPosted` = 'yes')",'GroupBy'=>'TypeID'));
#-------------------------------------------------------------------------------
