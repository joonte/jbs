...
if(Is_Array($Data)){
	#-------------------------------------------------------------------------------
	Debug(SPrintF('[comp/www/API/Logon]: Data = %s',print_r($Data,true)));
	#-------------------------------------------------------------------------------
	$Email		= (string) $Data['Email'];
	$Password	= (string) $Data['Password'];
	$IsRemember	= (boolean)@$Data['IsRemember'];
	#-------------------------------------------------------------------------------
}
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
// если запрос внутренний, считаем что галка установлена
if(!$IsExternal)
	$IsRemember = TRUE;
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
// если внутренний запрос, пароль не проверяем
if(!Preg_Match($Regulars['Password'],$Password) && $IsExternal)
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
// для внутреннего запроса пароль не проверяем
if($User['Watchword'] != Md5($Password) && $User['Watchword'] != Sha1($Password) && $IsExternal){
	#-------------------------------------------------------------------------------
	$UniqID = $User['UniqID'];
	#-------------------------------------------------------------------------------
