...
if(!$Count){
	...
	#-------------------------------------------------------------------------------
	$Out['Messages'][] = SPrintF('Заказов хостинга с доменом "%s" не найдено ни в каком статусе.',$Domain);
...
$Array = Array();
#-------------------------------------------------------------------------------
// если единый кошелёк есть и активен, закидываем его на первое место
if(IsSet($PaymentSystems['W1']['IsActive']) && $PaymentSystems['W1']['IsActive'])
	$Array[] = 'W1';
#-------------------------------------------------------------------------------
foreach(Array_Keys($PaymentSystems) as $PaymentSystemID){
	...
	if(!$PaymentSystems[$PaymentSystemID]['IsActive'])
		continue;
	#-------------------------------------------------------------------------------
	// у тиньковсской оплаты автоподтверждение аккаунта. получается кто угодно можетподтвердить, оплтатив
	// поэтому - пропускаем
	if($PaymentSystemID == 'Tinkoff')
		continue;
	#-------------------------------------------------------------------------------
...
if($PaymentSystem['IsContinuePaying']){
	...
	#-------------------------------------------------------------------------------
	// w1 его обсчитывает 
	// TODO подумать о вызове без него, чтобы когад надо с CSRF а когда не надо - без CSRF
	$Out['Button']['fields']['CSRF'] = $GLOBALS['CSRF'];
	#-------------------------------------------------------------------------------
}
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
