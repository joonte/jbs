...
switch(ValueOf($Ticket)){
  ...
  case 'array':
	...
	if($Ticket['UserID'] == $__USER['ID'] && $Ticket['Flags'] == "DenyAddMessage")
	  return new gException('DENY_ADD_MESSAGE','В этот тикет запрещено добавлять новые сообщения. Создайте новый и опишите в нём проблему.');
	#-----------------------------------------------------------------------
	#-----------------------------------------------------------------------
        if($Message){
          #---------------------------------------------------------------------
	  ...
	  #---------------------------------------------------------------------
        ...
        if($StatusID != $Ticket['StatusID']){
          #---------------------------------------------------------------------
          $Comp = Comp_Load('www/API/StatusSet',Array('ModeID'=>'Edesks','StatusID'=>$StatusID,'RowsIDs'=>$Ticket['ID']));
          #---------------------------------------------------------------------
          switch(ValueOf($Comp)){
            case 'error':
              return ERROR | @Trigger_Error(500);
