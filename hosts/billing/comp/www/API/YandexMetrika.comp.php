...
if($clientID){
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	// если оно есть в кэше, то не долбимся к базе
	$CacheID = SprintF('YandexMetrika-%s-%s',$GLOBALS['__USER']['ID'],$clientID);
	#-------------------------------------------------------------------------------
	$Result = CacheManager::get($CacheID);
	if($Result)     
		return Array('Status'=>'Ok','IsCached'=>TRUE);
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	if(!In_Array($clientID,$GLOBALS['__USER']['Params']['YM'])){
		...
		if(Is_Error($IsInsert))
			return ERROR | @Trigger_Error(500);
		#-------------------------------------------------------------------------------
		// кэшируем, чтобы одно и то же не писать во временную таблицу
