...
if(IsSet($Data->{'reply_message'}->{'id'})){
	...
	if($Attribs && $Attribs['TicketID'] != 0){
		...
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		// проверяем приложенные файлы и фотки
		if(IsSet($Data->{'attachments'})){
			#-------------------------------------------------------------------------------
			$Count = SizeOf($Data->{'attachments'}) - 1;
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/VK]: в сообщении (%s) приложенных файлов',$Count));
			#-------------------------------------------------------------------------------
			$FileType = $Data->{'attachments'}[$Count]->{'type'};
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/API/VK]: тип файла: %s',$FileType));
			#-------------------------------------------------------------------------------
			if($FileType == 'doc'){	// документ прислали
				#-------------------------------------------------------------------------------
				$Url = $Data->{'attachments'}[$Count]->{'doc'}->{'url'};
				$Name= $Data->{'attachments'}[$Count]->{'doc'}->{'title'};
				#-------------------------------------------------------------------------------
			}elseif($FileType == 'photo'){
				#-------------------------------------------------------------------------------
				// с фотами проблема, надо выковыривать с самым большим разрешением
				$Max = SizeOf($Data->{'attachments'}[$Count]->{'photo'}->{'sizes'}) - 1;
				#-------------------------------------------------------------------------------
				$Url = $Data->{'attachments'}[$Count]->{'photo'}->{'sizes'}[$Max]->{'url'};
				#-------------------------------------------------------------------------------
				$Name= BaseName($Url);
				#-------------------------------------------------------------------------------
			}
