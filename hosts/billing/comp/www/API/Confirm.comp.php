...
if(!In_Array($Method,Array_Keys($Config['Notifies']['Methods']))){
	#-------------------------------------------------------------------------------
	if(!XML_HTTP_REQUEST)
		return TemplateReplace('Generic.Error.Page',Array('TEXT'=>'Этот метод оповещения не существует'),FALSE);
	#-------------------------------------------------------------------------------
...
if(!$Config['Notifies']['Methods'][$Method]['IsActive']){
	#-------------------------------------------------------------------------------
	if(!XML_HTTP_REQUEST)
		return TemplateReplace('Generic.Error.Page',Array('TEXT'=>'Данный способ оповещения отключен администратором'),FALSE);
	#-------------------------------------------------------------------------------
...
if(!Preg_Match($Regulars[$Method],$Value)){
	#-------------------------------------------------------------------------------
	if(!XML_HTTP_REQUEST)
		return TemplateReplace('Generic.Error.Page',Array('TEXT'=>'Неверно указан адрес'),FALSE);
	#-------------------------------------------------------------------------------
...
if(!IsSet($Contact)){
	#-------------------------------------------------------------------------------
	if(!XML_HTTP_REQUEST)
		return TemplateReplace('Generic.Error.Page',Array('TEXT'=>'Контакт с указанными параметрами не найден<BR/>Войдите в биллинговую систему от пользователя, контактный адрес которого вы подтверждаете, и попробуйте снова'),FALSE);
	#-------------------------------------------------------------------------------
...
if($Contact['Confirmed'] > 1672520400){
	#-------------------------------------------------------------------------------
	if(!XML_HTTP_REQUEST)
		return TemplateReplace('Generic.Error.Page',Array('TEXT'=>'Ваш контактный адрес уже подтверждён, не надо повторно вводить код или переходить по ссылке'),FALSE);
	#-------------------------------------------------------------------------------
	return new gException('ALREADY_CONFIRMED','Уже подтверждено');
	#-------------------------------------------------------------------------------
	Header(SPrintF('Location: /v2/Home'));
	#-------------------------------------------------------------------------------
	return NULL;
	#-------------------------------------------------------------------------------
}
...
if(!$Confirm && !$Code){
	...
	if(In_Array($Method,Array('MAX','Telegram','Viber','VKontakte'))){
		#-------------------------------------------------------------------------------
		...
		#-------------------------------------------------------------------------------
		if($Count)
			return new gException('CONFIRM_CODE_EXISTS_TG','Что-то пошло не так, нажмите кнопку "Подтвердить" ещё раз ...');
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		...
		foreach(Array_Keys($Replace) as $Key)
			$Server['Params']['ConfirmInstructions'] = Str_Replace($Key,$Replace[$Key],$Server['Params']['ConfirmInstructions']);
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		//SHOW_IM_INSTRUCTIONS - прибито в исключения обработчиков ошибок фронта
		return new gException('SHOW_IM_INSTRUCTIONS',$Server['Params']['ConfirmInstructions']);
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
	}
	#-------------------------------------------------------------------------------
...
#		$ConfirmShort = 1111;
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	return Array('Status'=>'Ok','ContactID'=>$ContactID);
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
}else{
	#-------------------------------------------------------------------------------
	...
	if($Code){
		...
		if($Code != $Contact['Confirmation']){
			#-------------------------------------------------------------------------------
			$DOM->AddAttribs('Body',Array('onload'=>"ShowAlert('Ссылка устарела, попробуйте подтвердить ещё раз','Warning');location.href = '/v2/Home';"));
			#-------------------------------------------------------------------------------
	...
	$DOM->AddAttribs('Body',Array('onload'=>"ShowAlert('Контактный адрес подтверждён'); location.href = '/v2/Home';"));
	#-------------------------------------------------------------------------------
	$DOM->AddChild('Into',$NoBody);
	#-------------------------------------------------------------------------------
	$Out = $DOM->Build();
