...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
// чтобы не частили с созданием тикетов
$CacheID = Md5($GLOBALS['__USER']['ID'] . $__FILE__);
#-------------------------------------------------------------------------------
$Result = CacheManager::get($CacheID);
if($Result)
	return gException('TOO_QUICK_EDESK_CREATE','Новые запросы нельзя создавать сликшом часто');
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Config = Config();
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
// короткую тему не пропускаем, если это не внутренний запрос
