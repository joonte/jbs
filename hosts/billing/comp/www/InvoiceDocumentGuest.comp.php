...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
# ищщем переданное доменное имя в заказах хостинга
$Where = Array(
		SPrintF("`Domain` = '%s' OR `Parked` LIKE '%s,%%' OR `Parked` LIKE '%%,%s,%%' OR `Parked` LIKE '%%%s'",$Domain,$Domain,$Domain,$Domain),
		'`StatusID` = "Suspended"'
		);
#-------------------------------------------------------------------------------
...
if(!$Count){
	...
	#-------------------------------------------------------------------------------
	$NoBody->AddChild(new Tag('P','Оплата невозможна - с таким доменом заказов не найдено, или он не заблокирован.'));
	$NoBody->AddChild(new Tag('P','Если вы владелец этого сайта, войдите в биллинговую систему и оплатите от своего имени.'));
	#-------------------------------------------------------------------------------
	$DOM->AddChild('Into',$NoBody);
	#-------------------------------------------------------------------------------
...
if($Count > 1){
	...
	#-------------------------------------------------------------------------------
	$NoBody->AddChild(new Tag('P','Оплата невозможна - с таким доменом найдено более одного заказа.'));
	$NoBody->AddChild(new Tag('P','Если вы владелец этого сайта, войдите в биллинговую систему и оплатите от своего имени.'));
	#-------------------------------------------------------------------------------
	$DOM->AddChild('Into',$NoBody);
	#-------------------------------------------------------------------------------
