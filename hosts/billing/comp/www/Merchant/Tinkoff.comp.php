...
if(!IsSet($Args['Amount']) || !$Args['Amount']){
	...
	if($Invoice['Summ'] != $Args['Amount'] / 100){
		...
		if($Args['Status'] == 'PARTIAL_REFUNDED'){
			...
			return "OK";
			#-------------------------------------------------------------------------------
		}elseif($Args['Status'] == 'REFUNDED'){
			#-------------------------------------------------------------------------------
			// произошёл полный возврат по счёту
			Debug(SPrintF("[comp/www/Merchant/Tinkoff]: полный возврат по счёту, частями"));
		}else{
			#-------------------------------------------------------------------------------
...
case 'CONFIRMED':
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	// если это оплата через СБП или картой МИР - автоматически подтверждаем юзера
	if($Args['Pan']){
		#-------------------------------------------------------------------------------
		$Reason = FALSE;
		#-------------------------------------------------------------------------------
		// оставляем только цифры и плюс
		$Pan = Preg_Replace('/[^0-9.+]/','',$Args['Pan']);
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		// проверка что это карта МИР
		$Pan = SubStr($Pan,0,4);
		#-------------------------------------------------------------------------------
		if(IntVal($Pan) > 2199 && IntVal($Pan) < 2205)
			$Reason = SPrintF("Оплата счёта (%s) картой МИР",$Invoice['ID']);
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		// проверка что телефон - вырезаем первые три символа, если они '+79' - это оплата СБП
		$Pan = SubStr($Pan,0,3);
		#-------------------------------------------------------------------------------
		if($Pan == '+79')
			$Reason = SPrintF("Оплата счёта (%s) по СБП",$Invoice['ID']);
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		if($Reason){
			#-------------------------------------------------------------------------------
			$Comp = Comp_Load('Users/Confirm',$Invoice['UserID'],$Reason);
			if(Is_Error($Comp))
				return ERROR | @Trigger_Error(500);
			#-------------------------------------------------------------------------------
		}
...
case 'REFUNDED':
	#-------------------------------------------------------------------------------
	// это вариант когда полную сумму вернули за несколько раз
	...
	#	return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	$Comp = Comp_Load('Formats/Currency',$Args['Amount']/100);
	if(Is_Error($Comp))
		return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	$Event = Array('UserID'=>$Invoice['UserID'],'PriorityID'=>'Billing','IsReaded'=>FALSE,'Text'=>SPrintF('Произведён возврат средств по счёту #%u, сумма (%s), процессинговый центр прислал статус "%s". Требуется списание в ручном режиме',$Number,$Comp,$Args['Status']));
	$Event = Comp_Load('Events/EventInsert',$Event);
	if(!$Event)
		return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
