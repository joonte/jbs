...
if($__USER['ID'] != $Ticket['UserID']){
	...
	if($LastMessage['UserID'] == $Ticket['UserID']){
		#-------------------------------------------------------------------------------
		// проверяем на здравствуйте, тогда надо написать добрый (утро/день/вечер)
		if(StriStr(Mb_StrToLower($LastMessage['Content']),'здравствуйте') !== FALSE){
			#-------------------------------------------------------------------------------
			if(Date("H") >= 04) {$Hi = "Доброе утро";}
			if(Date("H") >= 10) {$Hi = "Добрый день";}
			if(Date("H") >= 18) {$Hi = "Добрый вечер";}
			if(Date("H") >= 23 or Date("H") < 04) {$Hi = "Доброй ночи";}
			#-------------------------------------------------------------------------------
			Debug(SPrintF('[comp/www/TicketRead]: обнарухено "здравствуйте", автодобавление в окно ответа: $Hi = %s',$Hi));
			#-------------------------------------------------------------------------------
		}
		#-------------------------------------------------------------------------------
		// шаблоны приветствия
		...
		foreach(Array_Keys($HiMessages) as $Key){
			#-------------------------------------------------------------------------------
			if(StrStr(Mb_StrToLower($LastMessage['Content']),$Key) !== FALSE){
				#-------------------------------------------------------------------------------
				$Hi = $HiMessages[$Key];
				#-------------------------------------------------------------------------------
		...
		#-------------------------------------------------------------------------------
		// приветствие в тексте юзера не обнаружено, выдаём дефолт
		if(!IsSet($Hi) && $MaxMessageID['NumMessages'] == 1)
