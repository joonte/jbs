...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
Debug(print_r($__USER['Params'],true));
if($__USER['ID'] == 100){
	#-------------------------------------------------------------------------------
	$Comp = Comp_Load('Form/TextArea',Array('name'=>'EmailSign','rows'=>3,'prompt'=>'Подпись добавляемая в почтовые сообщения. Если не задана - добавляется обычная. Возможно использование html тегов',),@$__USER['Params']['EmailSign']);
	if(Is_Error($Comp))
		return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	$Table[] = Array(new Tag('TD',Array('colspan'=>2),'Подпись для почты'),new Tag('TD',Array('colspan'=>4),$Comp));
	#-------------------------------------------------------------------------------
}
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
