...
foreach(Array_Keys($Row) as $ColumnID){
	...
	if(Preg_Match('/blob/',$TypeID)){
		...
		if(Preg_Match('/(int|float)/',$TypeID)){
			...
			#-------------------------------------------------------------------------------
			$Attribs = Array('name'=>$ColumnID,'cols'=>40,'rows'=>3);
			#-------------------------------------------------------------------------------
			if(StrLen($Value) > 200)
				$Attribs['rows'] = 10;
			#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('onclick'=>"FormEdit('/Administrator/API/RowEdit','RowEditForm','Сохранение записи');",'type'=>'button','value'=>'Обновить'));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$Table[] = $Comp;
...
$DOM->AddChild('RowEditForm',$Comp);
#-------------------------------------------------------------------------------
$Comp = Comp_Load('Form/Input',Array('name'=>'RowID','type'=>'hidden','value'=>$Row['ID']));
if(Is_Error($Comp))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$DOM->AddChild('RowEditForm',$Comp);
