...
if(Is_Error($Tmp))
  return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$Folder = SPrintF('%s/tests/envelopes',$Tmp);
#-------------------------------------------------------------------------------
...
foreach($__ADMINISTRATOR['Profiles'] as $Executor){
  ...
  foreach($__CLIENT['Profiles'] as $Profile){
    #---------------------------------------------------------------------------
    echo SPrintF("Формирование конверта %s\n",$Profile['Name']);
    #---------------------------------------------------------------------------
    $Args = Array(
      #-------------------------------------------------------------------------
      'SessionID' => $__ADMINISTRATOR['SessionID'],
    ...
    #---------------------------------------------------------------------------
    $Answer = Http_Send('/Administrator/EnvelopeMake',$Settings,$Args);
    if(Is_Error($Answer))
      return ERROR | @Trigger_Error(500);
    #---------------------------------------------------------------------------
    $Answer = @JSON_Decode($Answer['Body'],TRUE);
    ...
    switch($Answer['Status']){
      ...
      case 'Ok':
        ...
        if(!$Envelope)
          return 'Ошибка загрузки конверта';
        #-----------------------------------------------------------------------
