...
if($MethodID || $ContactID){	// метод или конткт задан, вторая стадия, или редактирвоание
	...
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
	$Comp = Comp_Load('Form/Input',Array('type'=>'checkbox','name'=>'IsImmediate','id'=>'IsImmediate','value'=>'yes','prompt'=>'Присылать уведомления немедленно, если вы находитесь в биллинговой системе (что-то делаете или просто страница открыта)'));
	if(Is_Error($Comp))
		return ERROR | @Trigger_Error(500);
	#-------------------------------------------------------------------------------
	if($Contact['IsImmediate'])
		$Comp->AddAttribs(Array('checked'=>'true'));
	#-------------------------------------------------------------------------------
	$Table[] = Array(new Tag('LABEL',Array('for'=>'IsImmediate'),'Уведомлять немедленно'),$Comp);
	#-------------------------------------------------------------------------------
	#-------------------------------------------------------------------------------
