...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
Debug(SPrintF('[comp/www/UserNotifiesSet]: $Contact = %s',print_r($Contact,true)));
if(!$Contact['Confirmed'])
	return new gException('CONTACT_NOT_FOUND',SPrintF('Контактный адрес (%u) не подтверждён. Для настройки уведомлений, его необходимо подтвердить',$Contact['Address']));
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Config = Config();
#-------------------------------------------------------------------------------
...
if(!$Config['Notifies']['Methods'][$Contact['MethodID']]['IsActive'])
	return new gException('METHOD_ID_DISABLED',SPrintF('Метод уведомления (%s) отключён и более не используется',$Contact['Address']));
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$DOM = new DOM();
#-------------------------------------------------------------------------------
...
if(Is_Error($DOM->Load('Window')))
	return ERROR | @Trigger_Error(500);
#-------------------------------------------------------------------------------
$DOM->AddText('Title',SPrintF('Настройка уведомлений для %s',$Contact['Address']));
#-------------------------------------------------------------------------------
...
if($Contact['MethodID'] == 'SMS'){
	...
	if(FloatVal($ServerSettings['Params']['ExceptionsPaidInvoices']) >= 0){
		...
		#-------------------------------------------------------------------------------
		#-------------------------------------------------------------------------------
		$Row2 = new Tag('TR',new Tag('TD',Array('colspan'=>2,'class'=>'Standard','style' => 'background-color:#FDF6D3;'),$Message));
		#-------------------------------------------------------------------------------
	}
	#-------------------------------------------------------------------------------
	if(FloatVal($ServerSettings['Params']['ExceptionsPaidInvoices']))
		UnSet($Row2);
	#-------------------------------------------------------------------------------
}
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
$Row = new Tag('TR',new Tag('TD',Array('class'=>'Head'),'Тип сообщения'),new Tag('TD',Array('class'=>'Head'),$Contact['MethodID']));
#-------------------------------------------------------------------------------
