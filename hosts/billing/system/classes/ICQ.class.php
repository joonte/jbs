...
class ICQ implements Dispatcher {
    ...
    public function send(Msg $msg) {
        ...
        $templateFile = SPrintF('Notifies/ICQ/%s.tpl', $msg->getTemplate());
        ...
        if(!$User['ICQ'])
            return new gException('RECIPIENT_UIN_NOT_FILLED','Получатель не заполнил UIN службы ICQ');

        $IsAdd = Comp_Load('www/Administrator/API/TaskEdit',Array('UserID'=>$User['ID'],'TypeID'=>'ICQ','Params'=>Array($User['ICQ'],$message,$User['ID'])));
